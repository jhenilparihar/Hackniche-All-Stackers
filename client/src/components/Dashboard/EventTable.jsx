...
class EventTable extends Component {
  ...
  tick() {
    ...
    dt.forEach((cert) => {
      const tr = document.querySelector(".tr" + cert.certid.toNumber());
      if (ckList.includes(cert.certid.toNumber()) == false) {
        tr.style.backgroundColor = "white";
        document.querySelector(".chk" + cert.certid.toNumber()).checked = false;
      } else {
        tr.style.backgroundColor = "#e6f7ff";
        document.querySelector(".chk" + cert.certid.toNumber()).checked = true;
      }
    });
  ...
  tog = async () => {
    ...
    if (allCheck.checked == true) {
      ...
      var chkList = [];
      data1.forEach((cert) => {
        chkList.push(cert.certid.toNumber());
      });
                  ...
                  {this.props.data.map((item) => (
                    <tr className={"commontr tr" + item.certid.toNumber()}>
                      <td>
                        <input
                          type="checkbox"
                          className={"commonChk  chk" + item.certid.toNumber()}
                          onChange={() => this.send(item, item.certid.toNumber())}
                        />
                      </td>
                      <td>
